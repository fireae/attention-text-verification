...
def train(loss, init_fn, hparams):
    ...
    if FLAGS.sync_replicas:
        ...
        sync_optimizer = tf.LegacySyncReplicasOptimizer(
                opt = optimizer,
...
def main(_):
    ...
    with tf.device(device_setter):
        ...
        predicted_probs = model.create_base(data.images, data.labels)
        total_loss = model.create_loss(predicted_probs, data.probs)
